...
export default function GestionPlantillas() {
  ...
  async function duplicarPlantilla(p: Plantilla) {
    ...
    try {
      ...
      for (const s of (secciones || [])) {
        ...
        if (nuevaSeccion) {
          ...
          if (items && items.length > 0) {
            const { error: eItems } = await supabase.from('plantilla_items').insert(
              items.map(i => ({
            ...
            // Una copia con secciones vacías es una lista de comprobación que
            // no comprueba nada, y quien la use creerá estar pasando el control.
      ...
      if (informe) {
        const { error: eInf } = await supabase.from('plantilla_informes').insert({
          plantilla_id: copia.id,
        ...
        if (eInf) toast.error('Copia creada, pero sin su configuracion de informe: ' + eInf.message)
      }
