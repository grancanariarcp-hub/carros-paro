...
export default function AdminPage() {
  ...
  const alertasFiltradas = alertas.filter(a => {
    ...
    return a.tipo === filtroAlerta
  })

  const carrosFiltrados = carros.filter(c => {
    if (filtroTipoCarro === 'todos') return true
      ...
      <div className="content">
        ...
        {tab === 'resumen' && <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button className="card flex items-center gap-2 cursor-pointer active:bg-gray-50"
              ...
              <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                <svg className="w-4 h-4 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14" strokeWidth={2}/></svg>
              </div>
              <span className="text-sm font-semibold">Nuevo carro</span>
            </button>
            <button className="card flex items-center gap-2 cursor-pointer active:bg-gray-50"
              onClick={() => router.push('/admin/nuevo-usuario')}>
              <div className="w-8 h-8 rounded-lg bg-purple-100 flex items-center justify-center flex-shrink-0">
                <svg className="w-4 h-4 text-purple-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" strokeWidth={2}/><circle cx="12" cy="7" r="4" strokeWidth={2}/></svg>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="card text-center">
              ...
              <div className="text-xs text-gray-500 mt-1">Operativos</div>
            </div>
            <div className="card text-center">
              <div className="text-2xl font-bold text-amber-600">{stats.condicionales}</div>
              <div className="text-xs text-gray-500 mt-1">Condicionales</div>
            </div>
            <div className="card text-center">
              <div className="text-2xl font-bold text-red-700">{stats.no_operativos}</div>
              <div className="text-xs text-gray-500 mt-1">No operativos</div>
            </div>
            <div className="card text-center">
              <div className="text-2xl font-bold text-gray-700">{stats.controles_vencidos}</div>
              <div className="text-xs text-gray-500 mt-1">Controles vencidos</div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setTab('alertas')}
                    className="text-xs text-red-600 font-semibold">Ver todas →</button>
                </div>
              </div>
          ...
          <div className="card">
            ...
            {inspecciones.length === 0 && (
              <div className="text-xs text-gray-400 text-center py-4">Sin controles registrados aún</div>
            )}
            {inspecciones.map(ins => {
              const e = estadoColor(ins.resultado)
              return (
                <div key={ins.id} className="row-item cursor-pointer"
                  onClick={() => router.push(`/carro/${ins.carro_id}/resultado/${ins.id}`)}>
        ...
        {tab === 'alertas' && <>
          <div className="card">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {([
                ...
                <button key={val}
                  onClick={() => setFiltroAlerta(val)}
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                    filtroAlerta === val ? cls + ' border-current' : 'bg-white text-gray-400 border-gray-200'
                  }`}
                >{label}</button>
              ))}
            </div>
          ...
          {alertasFiltradas.length === 0 ? (
            ...
            alertasFiltradas.map(a => {
              const colorTipo = a.tipo === 'no_operativo' ? 'border-red-300 bg-red-50'
                : a.tipo === 'vencimiento' ? 'border-amber-300 bg-amber-50'
                : 'border-orange-300 bg-orange-50'
              const badgeTipo = a.tipo === 'no_operativo' ? 'bg-red-100 text-red-700'
                : a.tipo === 'vencimiento' ? 'bg-amber-100 text-amber-700'
                ...
                <div key={a.id} className={`card ${colorTipo}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      onClick={() => marcarAlertaResuelta(a.id)}>
                      Marcar resuelta
                    </button>
                  </div>
                </div>
        ...
        {tab === 'carros' && <>
          ...
          {tiposCarro.length > 1 && (
            <div className="flex gap-1.5 flex-wrap">
              <button
                onClick={() => setFiltroTipoCarro('todos')}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${filtroTipoCarro === 'todos' ? 'bg-blue-100 text-blue-700 border-blue-300' : 'bg-white text-gray-400 border-gray-200'}`}
              >Todos</button>
              {tiposCarro.map(t => (
                <button key={t}
                  onClick={() => setFiltroTipoCarro(t)}
          ...
          <div className="card">
            {carrosFiltrados.length === 0 && (
              <div className="text-xs text-gray-400 text-center py-6">No hay carros creados aún</div>
            )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-400 flex items-center gap-1">
                      ...
                      {!controlVencido && c.proximo_control && (
                        <span>· Control: {formatFecha(c.proximo_control)}</span>
                      )}
                    </div>
                  </div>
                  <span className={`badge ${e.bg} ${e.text}`}>{e.label}</span>
        ...
        {tab === 'usuarios' && <>
          <div className="grid grid-cols-2 gap-2">
            <button className="btn-primary" onClick={() => router.push('/admin/usuarios')}>
              👥 Gestionar usuarios
            </button>
            <button className="btn-secondary" onClick={() => router.push('/admin/nuevo-usuario')}>
              + Crear usuario
            </button>
          </div>
