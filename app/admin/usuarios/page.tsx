...
export default function AdminUsuariosPage() {
  ...
  async function cargarDatos() {
    ...
    if (p.hospital_id) {
      const { data: h } = await supabase.from('hospitales').select('*').eq('id', p.hospital_id).maybeSingle()
      setHospital(h)
    }
  ...
  async function crearUsuario() {
    ...
    setGuardando(true)
    try {
      // Usar signUp en vez de auth.admin.createUser (no requiere service role)
      ...
      toast.success(`Usuario "${form.nombre}" creado. Recibirá un email para confirmar su cuenta.`)
      setModal(null)
      resetForm()
      await cargarDatos()
    } catch (err: any) {
